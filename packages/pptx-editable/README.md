...
Globally, once, on macOS, Windows or Linux with Node 22.18 or newer, straight from GitHub's archive of the release tag, no clone and no git needed:
...
Releases are tags named `pptx-editable-v<version>` (https://github.com/stn1slv/slidev-extensions/tags); the archive of `main` installs the latest commit instead: `npm i -g https://github.com/stn1slv/slidev-extensions/archive/refs/heads/main.tar.gz`. The package is not published to npm; it is a bridge until the upstream pull request ships.
